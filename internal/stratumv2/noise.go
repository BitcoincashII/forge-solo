...
func ServerHandshake(conn net.Conn, config *NoiseConfig) (*NoiseConn, error) {
	...
	// Bound the handshake in time so a connection that opens but never completes
	// the handshake cannot hold the accept goroutine and resources indefinitely.
	...
	// Handshake complete; clear the handshake deadline (the mining loop manages
	// its own timeouts thereafter).
...
func (nc *NoiseConn) Read(b []byte) (int, error) {
	...
	// Return any plaintext left over from a previous decrypt before reading a new
	// frame; otherwise a decrypted message larger than the caller's buffer would be
	// buffered and never delivered (silent data loss / stream corruption).
	if len(nc.readBuf) > 0 {
		n := copy(b, nc.readBuf)
