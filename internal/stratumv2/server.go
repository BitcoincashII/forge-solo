...
func (s *Server) handleClient(client *Client) {
	// HIGH FIX: Read timeout to prevent Slowloris-style DoS attacks
	...
	for {
		...
		// HIGH FIX: Set read deadline before each read to prevent connection holding
		client.noiseConn.SetReadDeadline(time.Now().Add(readTimeout))
...
func (s *Server) handleSubmitSharesStandard(client *Client, frame *Frame) error {
	...
	// HIGH FIX: Verify channel ownership before processing share
	if channel.ClientID != client.ID {
		s.logger.Warn("Share submitted to channel owned by another client",
			zap.Uint32("channel_id", channelID),
			zap.String("client_id", client.ID),
			zap.String("channel_owner", channel.ClientID))
		return s.sendSubmitSharesError(client, channelID, sequenceNo, "not-authorized")
	}
...
func (s *Server) handleSubmitSharesExtended(client *Client, frame *Frame) error {
	...
	// HIGH FIX: Verify channel ownership before processing share
	if channel.ClientID != client.ID {
		s.logger.Warn("Extended share submitted to channel owned by another client",
			zap.Uint32("channel_id", channelID),
			zap.String("client_id", client.ID),
			zap.String("channel_owner", channel.ClientID))
		return s.sendSubmitSharesError(client, channelID, sequenceNo, "not-authorized")
...
func (s *Server) handleUpdateChannel(client *Client, frame *Frame) error {
	...
	// HIGH FIX: Verify channel ownership before allowing update
	if channel.ClientID != client.ID {
		s.logger.Warn("Unauthorized channel update attempt",
			zap.Uint32("channel_id", channelID),
			zap.String("client_id", client.ID),
			zap.String("channel_owner", channel.ClientID))
		return s.sendUpdateChannelError(client, channelID, "not-authorized")
	}
...
func (s *Server) handleCloseChannel(client *Client, frame *Frame) error {
	...
	// CRITICAL FIX: Verify channel ownership before allowing close
	// Prevents malicious clients from closing other miners' channels
	chInterface, exists := s.channels.Load(channelID)
	if !exists {
		s.logger.Warn("Attempt to close non-existent channel",
			zap.Uint32("channel_id", channelID),
			zap.String("client_id", client.ID))
	...
	channel := chInterface.(*MiningChannel)
	if channel.ClientID != client.ID {
		s.logger.Warn("Unauthorized channel close attempt",
			zap.Uint32("channel_id", channelID),
			zap.String("client_id", client.ID),
			zap.String("channel_owner", channel.ClientID))
		return fmt.Errorf("not authorized to close channel %d", channelID)
	}
...
func (s *Server) BroadcastJob(job *MiningJobState) {
	...
	// HIGH FIX: Clean up old jobs to prevent unbounded memory growth
	s.cleanupJobHistory(100)
...
// cleanupJobHistory removes old jobs to prevent unbounded memory growth
// HIGH FIX: Prevents memory exhaustion from accumulating job history
func (s *Server) cleanupJobHistory(maxJobs int) {
	...
	// Delete excess jobs (keeping newest by not deleting current job)
	toDelete := count - maxJobs
	deleted := 0
	currentJob := s.currentJob.Load()

	s.jobHistory.Range(func(key, value interface{}) bool {
		if deleted >= toDelete {
			return false
		}
		// Don't delete the current job
		if currentJob != nil {
			if job, ok := currentJob.(*MiningJobState); ok && key.(uint32) == job.JobID {
				return true
			}
		}
		s.jobHistory.Delete(key)
		deleted++
		return true
	})
}
